...
import { isSameOrderId } from '../common/commonUtil';
import {
...
const buildNewPosition = (newPosition, operatingHourData) => {
    ...
    const output = {
        ...
        szRate: parseInt(newPosition[SZ_RATE_INDEX]),
...
const useUpdateData = (): ReturnType => {
    ...
    const handleSLCancel = useCallback(({ szOrgCustItem, data }) => {
        const newArr = data.map((d) => {
            if (isSameOrderId(d[SZ_STOP_ITEM_INDEX_IN_OPEN_POSITIONS], szOrgCustItem)) {
                const pipLowest = symbols[d[SZ_CUR_NO_INDEX_IN_OPEN_POSITIONS]]
                    ? symbols[d[SZ_CUR_NO_INDEX_IN_OPEN_POSITIONS]].pipLowest
                    : 2;
                d[STOP_PRC_INDEX_IN_OPEN_POSITIONS] = formatNumber(0, pipLowest as number);
                return d;
            } else {
                return d;
            }
    ...
    const handleLimitCancelOnTradeServer = useCallback(({ szOrgCustItem, data }) => {
        const newArr = data.map((d) => {
            if (isSameOrderId(d[SZ_LIMIT_ITEM_INDEX_IN_OPEN_POSITIONS], szOrgCustItem)) {
                ...
                const pipLowest = symbols[d[SZ_CUR_NO_INDEX_IN_OPEN_POSITIONS]].PIP_LOWEST;
                d[LIMIT_PRC_INDEX_IN_OPEN_POSITIONS] = formatNumber(0, pipLowest as number);
    ...
    const parseData = useCallback(
        (data) => {
            ...
            return data.map((d) => {
                const newD = [...d];
                const pipLowest = symbols[newD[SZ_CUR_NO_INDEX_IN_OPEN_POSITIONS]]
                    ? symbols[newD[SZ_CUR_NO_INDEX_IN_OPEN_POSITIONS]].pipLowest
                    : 2;
